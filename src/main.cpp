...
#if defined(NDEBUG)
#error "Ventura cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zZVT spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zZVT spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zZVT spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight < 201 && nHeight > 0) {
        nSubsidy = 1 * COIN;
    } else if (nHeight < 35001 && nHeight > 200) {
        nSubsidy = 16 * COIN;
    } else if (nHeight < 125001 && nHeight > 35000) {
        nSubsidy = 14 * COIN;
    } else if (nHeight < 300001 && nHeight > 125000) {
        nSubsidy = 12 * COIN;
    } else if (nHeight < 500001 && nHeight > 300000) {
        nSubsidy = 10 * COIN;
    } else if (nHeight < 725001 && nHeight > 500000) {
        nSubsidy = 7 * COIN;
    } else if (nHeight < 975001 && nHeight > 725000) {
        nSubsidy = 3 * COIN;
    } else if (nHeight < 1150001 && nHeight > 975000) {
        nSubsidy = 2 * COIN;
    } else if (nHeight < 2155081 && nHeight > 1150000) {
        nSubsidy = 1 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Ventura
...
{
    RenameThread("ventura-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZVENTURAMinted()
{
...
void RecalculateZVENTURASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zZVT supply
...
bool RecalculateVENTURASupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zZVT with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zZVTSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert VENTURA to zZVT
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zZVT spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zZVT serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zZVT mints and %d zZVT spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Ventura: We use certain sporks during IBD, so check to see if they are
