...
#include "zventuracontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zZVT ought to be enough for anybody." - Bill Gates, 2017
    ui->zZVTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzZVTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzZVT->setEnabled(false);
        ui->pushButtonMintzZVT->setToolTip(tr("zZVT is currently disabled due to maintenance."));

        ui->pushButtonSpendzZVT->setEnabled(false);
        ui->pushButtonSpendzZVT->setToolTip(tr("zZVT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zZVTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZVT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZVT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zZVT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zZVT in ") +
...
void PrivacyDialog::on_pushButtonSpendzZVT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZVT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zZVT
        sendzZVT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zZVT
    sendzZVT();
}

void PrivacyDialog::on_pushButtonZVenturaControl_clicked()
{
    ZVenturaControlDialog* zZVTControl = new ZVenturaControlDialog(this);
    zZVTControl->setModel(walletModel);
    zZVTControl->exec();
}

void PrivacyDialog::setZVenturaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzZVTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzZVT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ventura Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zZVTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zZVT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VENTURA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zZVT</b>";
    ...
    // use mints from zZVT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVenturaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZVenturaControlDialog::GetSelectedMints();
    }
    ...
    // Clear zventura selector in case it was used
    ZVenturaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zZVT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Ventura, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zZVT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zZVT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zZVT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zZVT "));
    ui->labelzZVTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzZVTSyncStatus->setVisible(fShow);
}
