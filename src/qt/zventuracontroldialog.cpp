...
#include "ui_zventuracontroldialog.h"
...
std::list<std::string> ZVenturaControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZVenturaControlDialog::listMints;

ZVenturaControlDialog::ZVenturaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVenturaControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZVenturaControlDialog::~ZVenturaControlDialog()
{
    delete ui;
}

void ZVenturaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVenturaControlDialog::updateList()
{
...
void ZVenturaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVenturaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZVentura_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVenturaControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZVenturaControlDialog::GetSelectedMints()
{
...
void ZVenturaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
